...
def performScan(
    ...
    totalNumberOfRays = (
        xRange.size * yRange.size * int((frameEnd - frameStart + 1) / frameStep)
    )
                ...
                if simulateWaterProfile:
                    ...
                    for layerIndex in range(firstValueBelowSensor, len(depthList) + 1):
                        ...
                        if layerIndex == len(depthList):
                            ...
                            newRange = (oldHeight - depthList[layerIndex][0]) / np.cos(
                                angle
                            )
                            ...
                            if debugOutput:
                                print(
                                    sensorHeight,
                                    depthList[layerIndex][0],
                                    depthList[layerIndex][1],
                                    angle,
                                    remainingDistance,
                                    newRange,
                                )
                        ...
                        closestHit = castRay(
                            ...
                            internalOrigin,
                            direction,
                            min(remainingDistance, newRange),
                            materialMappings,
                            depsgraph,
                            debugLines,
                            debugOutput,
                            remainingSourceLevel,
                            noiseLevel,
                            directivityIndex,
                            processingGain,
                            receptionThreshold,
                        ...
                        if debugLines:
                            generic.addLine(
                                internalOrigin,
                                internalOrigin
                                + direction * min(remainingDistance, newRange),
                            )
                        ...
                        if closestHit is None:
                            ...
                            newDirection = n * (
                                ...
                                1
                                - (n**2)
                                * (normal.cross(direction) @ normal.cross(direction))
                            )
                            ...
                            denominator = p2 * v2 * np.cos(
                                incidentAngle
                            ) + p1 * v1 * np.cos(refractionAngle)
                            transmission = (
                                4
                                * p1
                                * v1
